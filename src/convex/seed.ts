...
function avatar(i: number) {
  return `https://api.dicebear.com/9.x/thumbs/svg?seed=nanban-${i}`;
}
...
export const run = mutation({
  ...
  handler: async (ctx, { wipe = false, force = false }) => {
    ...
    const orgID: Id<'orgs'> = existingOrg?._id ?? (await ctx.db.insert('orgs', { name: 'Zurat, Inc.', slug: orgSlug }));
    ...
    for (const slug of projectSlugs) {
      ...
      const memberIds =
        slug === 'nanban-core' ? allUserIds : allUserIds.filter((_, idx) => (idx + String(slug).length) % 2 === 0);
    ...
    for (const slug of projectSlugs) {
      ...
      for (let i = 0; i < wikiTitles.length; i++) {
        ...
        const existing = await ctx.db
          .query('wiki')
          .withIndex('by_project_title', (q: any) => q.eq('projectID', projectID).eq('title', title))
          .first();
    ...
    const existingEveryone = await ctx.db
      .query('chats')
      .withIndex('by_org_type_name', (q: any) => q.eq('orgID', orgID).eq('type', 'group').eq('name', 'Everyone'))
      .first();

    const everyoneChatId: Id<'chats'> =
      existingEveryone?._id ?? (await ctx.db.insert('chats', { orgID, type: 'group', name: 'Everyone' }));

    const existingAI = await ctx.db
      .query('chats')
      .withIndex('by_org_type_name', (q: any) => q.eq('orgID', orgID).eq('type', 'ai').eq('name', 'AI'))
      .first();

    const aiChatId: Id<'chats'> = existingAI?._id ?? (await ctx.db.insert('chats', { orgID, type: 'ai', name: 'AI' }));
